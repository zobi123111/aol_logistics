...
@section('sub-title', __('messages.Supplier User') . ' | Company: ' . $supplier->company_name . ' | User: ' . $user->fname . ' ' . $user->lname)
@extends('layout.app')
@section('content')
    ...
    <div class="card card-container">
        <div class="card-body">
            <form action="{{ route('supplier_users.update', [encode_id($supplier->id), encode_id($user->id)]) }}" method="POST">
                ...
                <button type="submit" class="btn btn-primary btn_primary_color">{{ __('messages.Update User') }} </button>
            </form>
        </div>
    </div>
</div>
